...
function(libname, pkgname) {
    ...
    jrever <- .Call("RegGetStrValue",c(key,"CurrentVersion"), PACKAGE="JavaGD")
    ...
    if (!is.null(jrever)) {
        ...
        micro <- .Call("RegGetStrValue", c(key,"MicroVersion"), PACKAGE="JavaGD")
        if (!is.null(micro)) dispver <- paste(dispver,micro,sep=".")
#       cat("using Java Runtime version",dispver,"\n")
        javahome <- .Call("RegGetStrValue",c(key,"JavaHome"), PACKAGE="JavaGD")
        if (!is.null(javahome)) { # ok, let's try to get the real lib path
        p <- .Call("RegGetStrValue",c(key,"RuntimeLib"), PACKAGE="JavaGD")
        if (!is.null(p)) {
...
#    else cat("using JAVA_HOME =", javahome, "\n")
    ...
    .C("setJavaGDClassPath",paste(.javaGD.jar.file,Sys.getenv("CLASSPATH"),sep=";"), PACKAGE="JavaGD")
}
